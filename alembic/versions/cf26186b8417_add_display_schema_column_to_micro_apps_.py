"""add display_schema column to micro_apps table

Revision ID: cf26186b8417
Revises: a592347cd959
Create Date: 2025-08-15 08:37:59.366298
...
revision: str = 'cf26186b8417'
down_revision: Union[str, Sequence[str], None] = 'a592347cd959'
...
def upgrade() -> None:
    ...
    op.drop_constraint(op.f('uq_app_user_data'), 'micro_app_data', type_='unique')
    op.create_unique_constraint('uq_app_user_data', 'micro_app_data', ['app_id', 'user_id', 'id'])
    op.add_column('micro_apps', sa.Column('display_schema', postgresql.JSONB(astext_type=sa.Text()), nullable=True))
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_column('micro_apps', 'display_schema')
    op.drop_constraint('uq_app_user_data', 'micro_app_data', type_='unique')
    op.create_unique_constraint(op.f('uq_app_user_data'), 'micro_app_data', ['app_id', 'user_id'], postgresql_nulls_not_distinct=False)
