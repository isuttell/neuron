...
def parse_dice_expression(expression: str) -> DiceRoll:
    """
    Parses a dice expression (e.g., '2d6+1') into its components: number of dice,
    sides of dice, and modifier.
...
class DiceToolArgs(BaseModel):
    dice: list[str] = Field(
        description=(
            "A list of strings representing dice expressions, e.g. '1d20', '1d4', "
            "'5d6+3'. The example '2d6+1' indicates rolling 2 dice, each with 6 sides, "
            "and adding a modifier of +1 to the total. The format consists of three "
            "parts: the number of dice (2), the letter 'd' to indicate dice, the "
            "number of sides on each die (6), and an optional modifier (+1). Must "
...
class DiceTool(BaseTool):
    name: str = "dice"
    description: str = """
Roll virtual dice and return the individual results of each dice and total. Use this
tool to simulate dice rolls for role-playing games, tabletop games, and other
applications such as helping the user make a choice. Always show the results of each
dice roll to the user. Always include at least one dice expression.
""".strip()
    ...
    def _run(self, dice: list[str]) -> str:
        ...
        result = [
            (
                f"Rolled {dice}: "
                f"results={'+'.join([str(result) for result in results])} "
                f"total={total}"
            )
