...
class SetThreadMemoryToolArgs(BaseModel):
    memory: str = Field(
        description="The full and complete memory to overwrite the existing thread memory. Do not paraphrase the memory."
    )
...
class SetThreadMemoryTool(BaseTool):
    ...
    async def _arun(self, memory: str, config: RunnableConfig) -> str:
        try:
